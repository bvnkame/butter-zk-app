...
import { JWTVerifyContractV2 } from './JWTVerifyContractV2.js';
...
async function deploy() {
  try {
    ...
    // const zkApp = new JWTVerifyContract(zkAppAddress);
    const zkApp = new JWTVerifyContractV2(zkAppAddress);
    ...
    console.log(`✅ JWTVerifyContract compiled in ${programCompileTime}s\n`);
    ...
    console.log('🔨 Compiling JWTVerifyContract...');
    const compileStartTime = Date.now();
    // await JWTVerifyContract.compile();
    await JWTVerifyContractV2.compile();
    ...
    console.log('📝 Deploy new transaction...');
    ...
    await deployTxn.sign([payerKey, zkAppKey]);
    const txnResult = await deployTxn.send();

    // -------------------------------------------------------

    // const verificationKey = (await JWTVerifyContractV2.compile()).verificationKey;
    // const contractAddress = zkAppKey.toPublicKey();
    // const currentNonce = Mina.getAccount(payerPublicKey).nonce

    //  console.log('📝 Deploy update transaction...');
    // const fee = 0.1e9; // 0.1 MINA fee
    // const upgradeTx = await Mina.transaction({ sender: payerPublicKey, fee, nonce: randomInt(0, 10000000) },
    //   async () => {
    //     const update = AccountUpdate.createSigned(contractAddress);
    //     update.account.verificationKey.set(verificationKey);
    //   }
    // );
    // await upgradeTx.sign([payerKey, zkAppKey]).prove();
    // const txnResult = await upgradeTx.send();
